            ...
            for length in [1, 2, 3, 4]:
                ...
                if move not in taken and move not in cord:
                    ...
                    for odirection in directions:
                        if is_golden_possible(move, board, direction, 'w') \
                            and is_golden_possible(move, board, odirection, 'w'):
                            is_golden = True
                            break
                    if not is_golden:
                        cord[move] = False
...
def initialize(size):
    ...
    border.goto(0,0)
    border.pendown()
    border.goto(0, size-1)
    border.goto(size - 1, size - 1)
    border.goto(size - 1, 0)
    border.goto(0,0)
    border.penup()
    border.ht()
